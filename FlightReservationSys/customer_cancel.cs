...
namespace FlightReservationSys
{
    ...
    {
        string ordb = "Data source=orcl;User Id=scott;Password=tiger;"; // or we can use a static OracleConnection in main calss
        ...
        static List<Tuple<int,int,string>> databasedata = new List<Tuple<int, int, string>>();
        ...
        {
            Query_form_data_from_DB();
        }
        void Query_form_data_from_DB()
        {
            OracleCommand cmd = new OracleCommand();
            cmd.Connection = conn;
            cmd.CommandText = "SELECT * FROM BOOKED_FLIGHTS";
            ...
            while (dr.Read())
            {
                ...
                cardNum = dr[2].ToString();
                databasedata.Add(new Tuple<int, int, string>(flightId, customerId, cardNum));
            }
            dr.Close();
        }
        ...
        List<object> get_flight_data(int currentComboSeletedIdx)
        {
            ...
            flight_data.Add(databasedata[currentComboSeletedIdx].Item3);    // adding card num
            return flight_data;
        }
        ...
        private void cancel_flight(object sender, EventArgs e)
        {
            ...
            {
                delete_booked_flight(databasedata[0].Item1, databasedata[0].Item2);
            }
        }
